...
uint32_t longlink_noop_cmdid();                     // 好大：心跳包的 request command id
uint32_t longlink_noop_resp_cmdid();                // 好大：心跳包的 response command id
uint32_t signal_keep_cmdid();                       // 好大：signal keeping command id
void longlink_noop_req_body(AutoBuffer& _body);     // 好大：即将发送心跳包 request 的回调
void longlink_noop_resp_body(AutoBuffer& _body);    // 好大：心跳包返回 response 的回调
